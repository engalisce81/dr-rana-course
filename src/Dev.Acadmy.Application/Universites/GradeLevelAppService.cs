...
namespace Dev.Acadmy.Universites
{
    public class GradeLevelAppService :ApplicationService
    {
        ...
        [Authorize(AcadmyPermissions.GradeLevels.View)]
        public async Task<PagedResultDto<GradeLevelDto>> GetListAsync(int pageNumber, int pageSize, string? search) => await _gradelevelManager.GetListAsync(pageNumber, pageSize, search);
