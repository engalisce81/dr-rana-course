...
namespace Dev.Acadmy.Questions
{
    public class QuestionManager:DomainService
    {
        ...
        public QuestionManager(IRepository<Quiz, Guid> quizRepository, IIdentityUserRepository userRepository, ICurrentUser currentUser, IRepository<QuestionBank> questionbankRepository, IRepository<QuestionType> questionTypeRepository, IMapper mapper, IRepository<Question> questionRepository)
        {
        ...
        public async Task<PagedResultDto<QuestionDto>> GetListAsync(int pageNumber, int pageSize, string? search)
        {
            ...
            if (!string.IsNullOrWhiteSpace(search)) queryable = queryable.Include(x=>x.Quiz).Include(x=>x.QuestionBank).Include(x=>x.QuestionType).Where(c => c.Title.Contains(search));
        ...
        {
            var roles = await _userRepository.GetRolesAsync(_currentUser.GetId());
            var questionBank = new List<QuestionBank>();
            if(roles.Any(x=>x.Name.ToUpper() == RoleConsts.Admin.ToUpper())) questionBank = await _questionbankRepository.GetListAsync();
            else questionBank = (await _questionbankRepository.GetQueryableAsync()).Where(x=>x.CreatorId == _currentUser.GetId()).ToList();
            var dtos = _mapper.Map<List<LookupDto>>(questionBank);
            var totalCount = await _questionTypeRepository.GetCountAsync();
            return new PagedResultDto<LookupDto>(totalCount, dtos);
        }

        public async Task<PagedResultDto<LookupDto>> GetListQuizzesAsync()
        {
            var roles = await _userRepository.GetRolesAsync(_currentUser.GetId());
            var quizzes = new List<Quiz>();
            if (roles.Any(x => x.Name.ToUpper() == RoleConsts.Admin.ToUpper())) quizzes = await _quizRepository.GetListAsync();
            else quizzes = await (await _quizRepository.GetQueryableAsync()).Where(x=>x.CreatorId ==_currentUser.GetId()).ToListAsync();
